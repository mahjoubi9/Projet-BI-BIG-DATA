...
public class PublicationController {
    ...
    public int countContry(String nameofcountry,String date1,String date2) {
        List<Publication> publications =  publicationRepository.findAll();
        AtomicInteger i= new AtomicInteger();
        publications.stream().filter(x->x.getAffiliations().equals(nameofcountry)&&(x.getPublication_Year()>=Integer.parseInt(date1))&&((x.getPublication_Year()<=Integer.parseInt(date2)))).forEach(x->{
i.getAndIncrement();
        });
        System.out.println("pay "+nameofcountry +" : count = "+i);
        System.out.println("Fin");
        /*for(int j=0;j<publications.size();j++){
            if(publications.get(j).getAffiliations().equals(nameofcountry)) {
                i++;
            }
        }*/
        return i.get();
   ...
   @GetMapping(path="/toutPublicationavecdate/{date1}/{date2}",produces="application/json")
   public List<String> toutPublicationavecdate(@PathVariable("date1") String date1, @PathVariable("date2") String date2){
       List<Publication> publications =  publicationRepository.findAll();
       System.out.println("date1 : "+date1);
       System.out.println("date2 : "+date2);
       System.out.println(getCode("Yemen"));
       String alldata = "";
       List<String> listalldata = new ArrayList<>();
       Map<String, String> mapcountry = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
       for(int j=0;j<publications.size();j++){

           if((!mapcountry.containsValue(publications.get(j).getAffiliations()))&&(publications.get(j).getPublication_Year()>=Integer.parseInt(date1))&&((publications.get(j).getPublication_Year()<=Integer.parseInt(date2)))){
            /*   alldata=alldata+"{"+ "id:\"" + getCode(publications.get(j).getAffiliations())+"\","+
                       "name:\""+ publications.get(j).getAffiliations()+"\","+
                       "value:"+ countContry(publications.get(j).getAffiliations(),date1,date2)+"}"+",";
               mapcountry.put(String.valueOf(j),publications.get(j).getAffiliations());*/
/////////////////////////////////////////////////////////////////////////
               listalldata.add("{"+ "\"id\":\"" + getCode(publications.get(j).getAffiliations())+"\","+
                       "\"name\":\""+ publications.get(j).getAffiliations()+"\","+
                       "\"value\":"+ countContry(publications.get(j).getAffiliations(),date1,date2)+"}");
               mapcountry.put(String.valueOf(j),publications.get(j).getAffiliations());
           }
    ...
    @GetMapping(path="/toutPublication",produces="application/json")
    public List<Publication> toutPublication(){
        List<Publication> publications =  publicationRepository.findAll();
        int i=0;
        for(int j=0;j<publications.size();j++){
            System.out.println(publications.get(j).getDocument_Title());

        }
   /*    AtomicInteger i = new AtomicInteger();
       int j= publications.size();
       publications.stream().filter(x->x.getPublication_Year()==0).forEach(x->{
        i.getAndIncrement();
        System.out.println("i="+i.get());
           System.out.println(x.getDocument_Title().toString());
       });
       System.out.println(i.get());
       System.out.println(j- i.get());*/
        return  publications;
    }
    @GetMapping(path="/getdatatexttitle",produces="application/json")
    public String getdatatexttitle(){
        ...
        for(int j=0;j<publications.size();j++){
            System.out.println(publications.get(j).getDocument_Title());
           alltitle=alltitle+" "+publications.get(j).getDocument_Title();
        }
       //System.out.println(alltitle);
        return  alltitle;
    }
   /* List<Publication> publications =  publicationRepository.findAll();
       publications.stream().forEach(x->{
        System.out.println(x.getDocument_Title().toString());
    });*/
