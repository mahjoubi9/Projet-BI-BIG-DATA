...
class LinksDetailsSpider(scrapy.Spider):
    name = 'links_details'
    #allowed_domains = ['v']
    #start_urls = ['http://v/']
    ...
    def parse(self, response):
        ...
        try:
            ...
            # Conference = response.css('span.event__code--text::text').get()#.//span[@class="event__code--text"]//text()
             ...
             print("################################ Conference")
            try:
             Total_Citations = response.xpath('//*[@id="pb-page-content"]/div/main/div[2]/article/div[1]/div[2]/div/div/div[6]/div/div[1]/div/div/a[1]/div/span//text()').get()
            # Conference = response.css('span.event__code--text::text').get()#.//span[@class="event__code--text"]//text()
            except:
        ...
        # for t in response.css('ul.rlist--inline'):
        #     #text= t.css('a::attr(href)').extract()[0]
        #     text= t.css('li.loa__item::text').get()
        #     yield{
        #         'link':text
        #     }
        #     cc = cc + 1
